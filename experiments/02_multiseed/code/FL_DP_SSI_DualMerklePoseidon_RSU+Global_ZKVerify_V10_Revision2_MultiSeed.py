#!/usr/bin/env python3
"""
Reviewer 1 / Round 2 / Comment 1 multi-seed FL-BC-IDS DP driver.

Publication-portable copy. Scientific configuration, training/evaluation
semantics, DP accounting, SSI evidence, Merkle commitments, and AnchorZKP
logic are preserved. Only repository/toolchain discovery and documentation
artifacts are hardened for the public archive layout.
"""
...
#   <repo>/environment/node_modules/
#   <repo>/experiments/02_multiseed/code/<this file>
#
# Historical development layout:
#   <project>/FL_IoV_*.py
#   <project>/node_modules/
#   <project>/Revision2/Reviewer1_Comment1_MultiSeed/code/<this file>
#
# FLBCIDS_REPO_ROOT, FLBCIDS_NODE_MODULES, and ANCHOR_ZKP_PTAU_PATH may be
...
def _helper_dir_for_root(root: Path) -> Path | None:
    public_core = root / "code" / "core"
    if all((public_core / name).is_file() for name in _REQUIRED_HELPERS):
        return public_core.resolve()

    if all((root / name).is_file() for name in _REQUIRED_HELPERS):
        return root.resolve()
...
def _find_project_root(start_dir: Path) -> Path:
    """Locate either the publication repository root or the legacy project root."""
    ...
    candidates.extend([start_dir, *start_dir.parents])
    ...
    for candidate in candidates:
        ...
        if _helper_dir_for_root(resolved) is not None:
            return resolved

    raise RuntimeError(
        "Could not locate the FL-BC-IDS repository/project root. "
        f"Started search from: {start_dir}. Expected either code/core/ "
        "containing the four FL_IoV_* V10 helpers (public archive layout), "
        "or the same helpers directly in the project root (legacy layout). "
        "Set FLBCIDS_REPO_ROOT to override discovery."
    )


def _resolve_node_modules(project_root: Path) -> Path:
    overrides = (
    ...
    raise RuntimeError(
        "Node dependencies were not found. Run `npm ci` in the repository "
        "environment/ directory, or set FLBCIDS_NODE_MODULES / "
        "ANCHOR_ZKP_NODE_MODULES to the installed node_modules directory."
    )


def _configure_ptau_if_present(project_root: Path, helper_dir: Path) -> None:
    """Set ANCHOR_ZKP_PTAU_PATH when the retained setup artifact is discoverable."""
    if os.getenv("ANCHOR_ZKP_PTAU_PATH", "").strip():
        return
    ...
    candidates = [
        ...
        project_root / "verification" / "groth16" / filename,
        project_root / "verification" / "groth16" / "canonical" / filename,
    ]

    for candidate in candidates:
        if candidate.is_file():
            os.environ["ANCHOR_ZKP_PTAU_PATH"] = str(candidate.resolve())
            return


_PROJECT_ROOT = _find_project_root(_SCRIPT_DIR)
_HELPER_DIR = _helper_dir_for_root(_PROJECT_ROOT)
if _HELPER_DIR is None:
    raise RuntimeError("Internal error: helper directory disappeared after root discovery.")

_NODE_MODULES = _resolve_node_modules(_PROJECT_ROOT)
_NODE_BIN = (_NODE_MODULES / ".bin").resolve()

# Make the V10 helper modules importable in both publication and legacy layouts.
_helper_dir_str = str(_HELPER_DIR)
if _helper_dir_str not in sys.path:
    sys.path.insert(0, _helper_dir_str)

# Share resolved paths with helper modules and Ray workers.
os.environ.setdefault("FLBCIDS_REPO_ROOT", str(_PROJECT_ROOT))
...
_configure_ptau_if_present(_PROJECT_ROOT, _HELPER_DIR)

# Make local Node CLIs discoverable inside the main process and Ray workers.
if _NODE_BIN.is_dir():
...
# ---------------------------------------------------------------------------
# Flower 1.23 uses Ray as the Simulation Engine backend. On Windows, rapidly
# tearing down one local Ray runtime and starting the next can leave transient
# GCS/port state behind. The study runs two RSUs sequentially in one process,
# so we force each RSU to start a fresh local Ray runtime, explicitly shut it
# down, limit the actor pool to the two required ClientApps, and retry only
# startup failures that occur before client training begins.
...
def _is_transient_ray_startup_failure(exc: BaseException) -> bool:
    text = _exception_chain_text(exc)
    return any(marker in text for marker in RAY_STARTUP_RETRY_MARKERS)


def _ray_cleanup_barrier() -> None:
    """Best-effort local Ray cleanup plus a short Windows process-exit barrier."""
    try:
        # ray.shutdown() is documented as safe even when called repeatedly.
        ray.shutdown()
    except Exception as cleanup_exc:
        logging.warning("Ray cleanup warning: %s", cleanup_exc)

    if os.name == "nt":
        time.sleep(RAY_WINDOWS_CLEANUP_GRACE_SEC)


def _build_ray_backend_config(num_supernodes: int) -> Dict:
    """
    Create a fresh local Ray backend sized only for the required ClientApps.

    address='local' prevents accidental attachment to stale local-cluster
    metadata. Restricting num_cpus to 2 CPUs per SuperNode prevents Flower
    from creating a much larger idle actor pool than this 2-vehicle RSU needs.
    """
    num_supernodes = max(1, int(num_supernodes))
    return {
        "init_args": {
            "address": "local",
            "include_dashboard": False,
            "num_cpus": int(2 * num_supernodes),
            "num_gpus": 0,
        },
        "client_resources": {
            "num_cpus": 2,
...
def _run_simulation_hardened(
    *,
    ...
    """
    Run one RSU simulation with cleanup and startup-only retries.

    Retries are intentionally restricted to Ray startup/GCS failures. A failure
    after ClientApps have begun training is propagated immediately so that the
    orchestrator can quarantine the whole stage before any rerun, preserving
    DP/evidence semantics.
    """
    backend_config = _build_ray_backend_config(num_supernodes)
    last_exc: BaseException | None = None

    for attempt in range(1, RAY_STARTUP_MAX_ATTEMPTS + 1):
        ...
        try:
            logging.info(
                ...
                int(num_supernodes),
                int(2 * max(1, int(num_supernodes))),
            )
            ...
            last_exc = exc
            transient_startup = _is_transient_ray_startup_failure(exc)

            if not transient_startup:
                raise

            if attempt >= RAY_STARTUP_MAX_ATTEMPTS:
                logging.error(
            ...
            # If Flower already terminated Ray, this is a harmless second cleanup.
    ...
    raise RuntimeError("Ray simulation retry loop exited unexpectedly.")
...
class VehicleClient(fl.client.Client):
    ...
    Each vehicle:
      ...
      - evaluates on the RSU-wide validation split during federated training,
      - exchanges XGBoost models with the RSU (Flower server).

    The TEST split is reserved for final post-training RSU and GLOBAL
    evaluation.
    """
    def __init__(
            self,
        ...
        self.feature_min, self.feature_max = feature_bounds
        ...
        # Entries contain both numeric audit fields and string paths/hashes.
    ...
    def fit(self, ins: FitIns) -> FitRes:
        ...
        try:
            ...
            # Omit early_stopping_rounds entirely when disabled so the
            # dp_xgboost call receives an int only when the option is active.
            if xgb_es_rounds is None:
                ...
                self.model = xgb.train(
                    ...
                    xgb_model=self.model,
                    evals=evals,
                    early_stopping_rounds=int(xgb_es_rounds),
                )
            ...
            metric_name = str(
                self.xgb_cfg.params.get("eval_metric", "auc")
            )
...
class XgbBaggingStrategy(FedXgbBagging):
    ...
    def aggregate_fit(
        ...
        try:
            ...
            if M > 0 and self.anchor_id_field:
                ...
                for client_proxy, fit_res in results_all:
                    ...
                    if not delta_bytes:
                        continue
                    # require SSI pubkey+sig and derive DID
                    ...
                    try:
                        ...
                        if (
                            ...
                            and str(v_from_metrics).strip() != ""
                        ):
                            veh_id = int(
                                float(str(v_from_metrics))
                            )
        ...
        try:
            ...
            logging.exception(
                "[ZKP][RSU %d] Round %d: "
                "RSU round verification pipeline FAILED",
                self.rsu_id,
                rnd,
            )
            if STRICT_MERKLE_AUDIT:
    ...
    def aggregate_evaluate(
        ...
        if metrics_records:
            try:
                weighted_metrics = weighted_average_eval_metrics(
                    metrics_records
                )

                for key, value in weighted_metrics.items():
                    if isinstance(value, (int, float)):
                        aggregated_metrics[str(key)] = float(value)

            except Exception as exc:
                logging.warning(
                    "RSU %d - Round %d: "
                    "failed to aggregate evaluation metrics: %s",
                    self.rsu_id,
                    rnd,
                    exc,
                )
...
def run_rsu_federated_learning(
    ...
    # Convert each vehicle's partition to concrete NumPy arrays once.
    ...
    def client_fn(context) -> fl.client.Client:
        ...
        # -------------------------------------------------------------------
        require_pubmask_obj: Any = getattr(
            azkp,
            "_require_pubmask_only_v15",
            None,
        )

        if not callable(require_pubmask_obj):
            # Hard fail if helpers are missing:
            # PUBMASK-only is required in the V15 plan.
            raise RuntimeError(
                "Missing azkp._require_pubmask_only_v15; "
                "update AnchorZKP utils to V15 PUBMASK-only."
            )

        require_pubmask_fn = cast(
            Callable[..., Any],
            require_pubmask_obj,
        )
    ...
    # The hardened wrapper starts a fresh local Ray runtime after these values
    # are set, so workers inherit them; client_fn also enforces the same pins.
    # -------------------------------------------------------------------
    env_v15 = {
    ...
    # IMPORTANT (V15): do not inject runtime_env. The hardened wrapper only
    # configures Ray lifecycle/resources; the process environment above is inherited
    # by the fresh local Ray workers, and client_fn still enforces the pins.
    ...
    # -------------------------------------------------------------------
    fp_fn_obj: Any = getattr(
        azkp,
        "ssi_preimage_fingerprint_v1",
        None,
    )

    if not callable(fp_fn_obj):
        raise RuntimeError(
    ...
    if not isinstance(fp_obj, dict):
        raise RuntimeError(
            "azkp.ssi_preimage_fingerprint_v1() "
            "must return a dict."
        )

    ssi_fp = {
        str(k): v
        for k, v in fp_obj.items()
    }
    # -------------------------------------------------------------------
...
if __name__ == "__main__":
    ...
    # multi-seed, independently repartitioned evaluation of the principal
    # compact FL-BC-IDS learning configuration.
    #
    # IMPORTANT:
    #   - The original driver remains untouched.
    #   - Each seed reads the seed-specific preprocessing artifacts created
    #     under the Revision-2 folder.
    #   - Each run writes to its own isolated full_dp directory.
    #   - ANCHOR_SEED remains fixed at 42 so the anchor-sampling procedure
    #     itself is not an additional source of randomness. The candidate
    #     anchor pool is the seed-specific validation split, so anchor
    #     membership may legitimately differ across experimental seeds.
    # -----------------------------------------------------------------------
    parser = argparse.ArgumentParser(
        description=(
            "FL-BC-IDS compact DP run for Reviewer 1 Round-2 multi-seed "
    ...
    parser.add_argument(
        ...
        default=42,
        help=(
            "Experimental seed used by XGBoost and the hierarchical "
            "RSU/vehicle partitioning. The preprocessing stage must have "
            "already been run with the same seed."
        ),
    )
    parser.add_argument(
        "--revision-root",
    ...
    parser.add_argument("--num-local-rounds", type=int, default=10)
    args = parser.parse_args()

    # The retained Round-2 execution contract launched every child interpreter
    # with PYTHONHASHSEED=0. Require the same process-start setting for direct
    # execution so hash-based ordering cannot silently diverge from the
    # validated multi-seed runs.
    if os.environ.get("PYTHONHASHSEED") != "0":
        raise RuntimeError(
            "This multi-seed DP driver requires PYTHONHASHSEED=0 at interpreter "
            "startup. Run it through Reviewer1_Comment1_Run_MultiSeed.py or set "
    ...
    # The preprocessing scripts created earlier use exactly this layout.
    ...
    # ---------------------------
    # Principal compact configuration
    # ---------------------------
    ...
    # Fixed sampling seed for the verification anchor procedure.
    # The source pool remains the seed-specific validation split.
    ANCHOR_SEED = 42

    ANCHOR_VERSION = "v1"
    ANCHOR_OVERWRITE = True

    # Every DP run gets an isolated Revision-2 directory.
    ...
    logging.info("=" * 100)
    logging.info("REVIEWER 1 ROUND-2 MULTI-SEED DP RUN")
    logging.info("Dataset              : %s", DATASET_NAME)
    logging.info("Experimental seed    : %d", EXPERIMENT_SEED)
    logging.info("Preprocessing input  : %s", PREPROC_DIR)
    logging.info("DP output directory  : %s", OUTPUT_DIR)
    logging.info(
        "Compact topology      : %d RSUs x %d vehicles, %d FL rounds, %d local rounds",
        NUM_RSUS,
        VEHICLES_PER_RSU,
        NUM_ROUNDS,
        NUM_LOCAL_ROUNDS,
    )
    logging.info("Anchor seed (fixed)   : %d", ANCHOR_SEED)
    logging.info("=" * 100)
    # -------------------------------------------------------------------
    ...
    def _build_onchain_export_index_v1(
        ...
        for rsu_id in rsu_ids_sorted:
            ...
            rounds_entry: Dict[str, Any] = {}

            rsu_entry: Dict[str, Any] = {
                "summary_json": {
                    "path": _p(rsu_summary_path),
                    "sha256": _sha256_file_hex_v1(
                        rsu_summary_path
                    ),
                },
                "rounds": rounds_entry,
    ...
    run_config = {
        ...
        "revision_context": {
            ...
            "anchor_seed_fixed": int(ANCHOR_SEED),
            "anchor_source_split_seed": int(EXPERIMENT_SEED),
        },
    ...
    if meta is None:
        raise RuntimeError(
            "AnchorMeta is None after normalization (unexpected)"
        )

    anchor_meta: azkp.AnchorMeta = meta
    ...
    _rsu_round_root_map_safe: Dict[int, int] = {
        ...
        for k, v in rsu_round_root_map.items()
    }

    if not _rsu_round_root_map_safe:
        _rsu_round_root_map_safe = {
            int(rid): 0
            for rid in rsu_ids_sorted
        }
