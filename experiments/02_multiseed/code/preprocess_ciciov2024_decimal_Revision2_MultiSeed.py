# preprocess_ciciov2024_final_final.py
...
OUT_DIR = Path()  # Set at runtime to a seed-specific Revision-2 directory.
...
def read_and_validate_one_decimal_file(csv_path: Path) -> Tuple[pd.DataFrame, Dict[str, Any]]:
    ...
    info: Dict[str, Any] = {
        ...
        "binary_counts": bincount_binary(
            df[BINARY_LABEL_COL].to_numpy(
                dtype=np.int8,
                copy=False,
            )
        ),
        "category_values": sorted(
            df[CATEGORY_COL].dropna().unique().tolist()
        ),
        "specific_class_values": sorted(
            df[SPECIFIC_CLASS_COL].dropna().unique().tolist()
        ),
...
def split_train_val_test(
    ...
    train_df: pd.DataFrame = df.take(
        idx_train,
        axis=0,
    ).reset_index(drop=True)

    val_df: pd.DataFrame = df.take(
        idx_val,
        axis=0,
    ).reset_index(drop=True)

    test_df: pd.DataFrame = df.take(
        idx_test,
        axis=0,
    ).reset_index(drop=True)

    train_y: np.ndarray = train_df[
        BINARY_LABEL_COL
    ].to_numpy(
        dtype=np.int8,
        copy=False,
    ...
    summary: Dict[str, Any] = {
        "train_before_balance": describe_binary_distribution(
            "TRAIN_BEFORE_BALANCE",
            train_y,
        ),
        "val": describe_binary_distribution(
            "VAL",
            val_y,
        ),
        "test": describe_binary_distribution(
            "TEST",
            test_y,
        ),
    }
...
def save_outputs_and_manifest(
    ...
    manifest = {
        ...
        "source_directory": str(DECIMAL_DIR),
        ...
        "output_files": {
            ...
            "preprocessor_joblib": str(preproc_path),
        },
        "file_hashes": {
...
def parse_args():
    parser = argparse.ArgumentParser(
        description=(
            "CICIoV2024 decimal preprocessing for Reviewer 1 Round-2 "
            "multi-seed evaluation. Each seed is written to an isolated "
            "Revision-2 folder."
        )
    )
    parser.add_argument(
        ...
        type=int,
        default=42,
        help="Experimental seed controlling train/validation/test splitting, oversampling, and final train shuffle.",
    )
    parser.add_argument(
        "--revision-root",
        type=Path,
    ...
    parser.add_argument(
        ...
        type=Path,
        default=DECIMAL_DIR,
        help="Directory containing the six original CICIoV2024 decimal CSV files.",
    )
...
# =============================================================================
def main() -> None:
    global RANDOM_STATE, OUT_DIR, DECIMAL_DIR, REVISION2_ROOT

    args = parse_args()
    RANDOM_STATE = int(args.seed)
    REVISION2_ROOT = Path(args.revision_root)
    DECIMAL_DIR = Path(args.decimal_dir)
    OUT_DIR = (
        REVISION2_ROOT
        / "CICIoV2024"
