#FL_DP_SSI_DualMerklePoseidon_RSU+Global_ZKVerify_V8c.py
#!/usr/bin/env python3
import inspect
import json
import os
...
# AnchorZKP: ensure Circom can resolve `include "circomlib/..."` (Windows/Ray)
_PROJECT_ROOT = Path(__file__).resolve().parent
# Prefer "where this script lives" (hard-coding breaks if you move the repo)
...
if not _NODE_MODULES.is_dir():
    raise RuntimeError(
        f"node_modules not found at {_NODE_MODULES}. Run: npm i circomlib circomlibjs (and npm i -D snarkjs if you rely on local snarkjs)."
    )
# Your utils SHOULD translate this into circom's official "-l <dir>" include search path. :contentReference[oaicite:1]{index=1}
os.environ["ANCHOR_ZKP_CIRCOM_LINK_LIBS"] = str(_NODE_MODULES)
os.environ["CIRCOM_LINK_LIBS"] = str(_NODE_MODULES)
# Make local node CLIs discoverable inside Ray processes (snarkjs/npx)
if _NODE_BIN.is_dir():
...
import FL_IoV_AnchorZKP_UtilsV9 as azkp          # AnchorSum ZK lane
import FL_IoV_CanonicalSpecV9 as canon           # ✅ Phase-1 canonical bytes authority
import FL_IoV_MerkleSSI_UtilsV9 as mssi           # ✅ Phase-2 dual leaves + dual roots + audit
...
# (see Flower docs/examples using context.node_config["partition-id"]). :contentReference[oaicite:0]{index=0}
...
from FL_IoV_DPxgb_UtilsV9 import (
    IoVDatasetConfig,
    RSUConfig,
    XGBoostConfig,
    setup_logging,
...
def run_rsu_federated_learning(
    ...
    def client_fn(context) -> fl.client.Client:
        ...
        def _resolve_partition_idx0_v15(ctx, *, num_parts_local: int, partition_id_base_default: int) -> tuple[
            ...
            # Flower examples use 0-based partition-id (0..num-partitions-1). :contentReference[oaicite:1]{index=1}
            ...
            if base_raw is None:
                ...
                # Flower docs/examples commonly use 0-based partition-id. :contentReference[oaicite:1]{index=1}
...
if __name__ == "__main__":
    ...
    DATASET_CONFIGS = {
        "CSECICIDS2018": {
            "preproc_dir": r"D:/PycharmProjects/IoV-FinalAttempt/preproc_fl_binary2",
        ...
        "CICIoV2024": {
            "preproc_dir": r"D:/PycharmProjects/IoV-FinalAttempt/preproc_ciciov2024_decimal_final",
    ...
    OUTPUT_DIR = r"D:/PycharmProjects/IoV-FinalAttempt/rsu_outputs_dp"
    # -------------------------------------------------------------------
