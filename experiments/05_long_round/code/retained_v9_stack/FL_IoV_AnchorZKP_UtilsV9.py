...
try:
    ...
    raise ImportError(
        "[AnchorZKP] Missing/failed import: FL_IoV_CanonicalSpecV8b.py.\n"
...
# Logging
# -----------------------------------------------------------------------------#
LOGGER = logging.getLogger("AnchorZKP.V8b")
...
DEFAULT_SCHEMA: str = "AnchorZKPArtifactV8b"
ENV_PROJECT_ROOT = "ANCHOR_ZKP_PROJECT_ROOT"
def _detect_project_root() -> Path:
    # 1) Explicit override (recommended for Ray/Windows)
    override = os.getenv(ENV_PROJECT_ROOT, "").strip()
    if override:
        p = Path(override).expanduser().resolve()
        if not p.is_dir():
            raise FileNotFoundError(f"[AnchorZKP] {ENV_PROJECT_ROOT} is not a directory: {p}")
        return p
    # 2) Best-effort: search upward for package.json or node_modules/circomlib
...
def precompile_anchorsum_groth16(
    *,
    ...
    if isinstance(out, dict):
        ...
        out["pre_dir"] = str(pre_dir)  # where .r1cs/.wasm/.sym and groth16 artifacts live in V8b
...
def _run_cmd(
    ...
    *,
    ...
    env = os.environ.copy()
    # Add BOTH cwd and project_root npm bins (important when cwd != project_root)
    for root in [project_root, Path(cwd)]:
        if root is None:
            continue
        nm_bin = Path(root) / "node_modules" / ".bin"
        if nm_bin.is_dir():
