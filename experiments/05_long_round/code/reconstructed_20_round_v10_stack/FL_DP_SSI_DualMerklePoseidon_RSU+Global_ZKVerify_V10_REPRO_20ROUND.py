#!/usr/bin/env python3
"""
Publication reproduction runner for the documented 20-round CSE-CIC-IDS2018 run.

This file is derived from the retained V10 implementation and is NOT asserted
to be the exact historical executable used for the original supplementary run.
"""
...
# AnchorZKP: locate the repository root while keeping this reconstructed
# directory as the source of the frozen V10 helper modules.
...
for _cand in _candidate_roots:
    _cand = _cand.resolve()
    if (_cand / "node_modules").is_dir():
        _PROJECT_ROOT = _cand
        break

if _PROJECT_ROOT is None:
    raise RuntimeError(
        "Could not locate the FL-BC-IDS repository root containing node_modules. "
        "Set FLBCIDS_REPO_ROOT to the repository root if automatic discovery fails."
    )
...
if __name__ == "__main__":
    setup_logging()
    logging.warning(
        "Running publication reconstruction of the documented 20-round experiment; "
        "this file is not asserted to be the exact historical executable."
    )
    ...
    OUTPUT_DIR = os.getenv(
        "FLBCIDS_DP_OUTPUT_DIR",
        os.path.join(str(_PROJECT_ROOT), "artifacts", "05_long_round", "rsu_outputs_dp"),
    )
    if not os.path.isabs(OUTPUT_DIR):
        OUTPUT_DIR = os.path.join(str(_PROJECT_ROOT), OUTPUT_DIR)
    # -------------------------------------------------------------------
