            ...
            if M > 0 and self.anchor_id_field:
                ...
                for client_proxy, fit_res in results_all:
                    ...
                    try:
                        ...
                        if int(m.get("ablation_digest_target", 0) or 0) == 1:
...
def run_public_input_agreement_check_v1(
    ...
    if not report["enabled"]:
        out_path = os.path.join(report_dir, "public_input_agreement_report.json")
        _write_json_pretty_v1(out_path, report)
        return report
    ...
    def _make_mismatch_list(
        names: List[str],
        expected_vals: List[str],
        exported_vals: List[str],
    ) -> List[Dict[str, Any]]:
        mismatches: List[Dict[str, Any]] = []
        max_len = max(len(names), len(expected_vals), len(exported_vals))
        for i in range(max_len):
            name = str(names[i]) if i < len(names) else f"<extra_{i}>"
            exp = str(expected_vals[i]) if i < len(expected_vals) else ""
            got = str(exported_vals[i]) if i < len(exported_vals) else ""
            if exp != got:
                mismatches.append(
                    {
                        "index": int(i),
    ...
    rsu_report: Dict[str, Any] = {
        ...
        "exported_public_inputs": [],
        "unresolved_public_input_names": [],
        "mismatches": [],
        "result": "fail",
    }
    ...
    try:
        ...
        proof_summary_path = _resolve_output_path_v1(
            output_dir,
            str(round_summary.get("rsu_anchor_artifact", "") or ""),
        )
        proof_summary = _read_json_v1(proof_summary_path)

        proof_path = _resolve_output_path_v1(
            output_dir,
            str(
                proof_summary.get("proof_artifact_copy_path", "")
                or proof_summary.get("proof_artifact_path", "")
                or ""
            ),
        )
        public_sidecar_path = _resolve_output_path_v1(
            output_dir,
            str(
                proof_summary.get("public_inputs_sidecar_copy_path", "")
                or proof_summary.get("public_inputs_sidecar_path", "")
                or ""
            ),
        )

        proof_obj = _read_json_v1(proof_path)
        public_sidecar_obj = _read_json_v1(public_sidecar_path)

        public_input_names = _spec_public_input_names_v1(spec_rsu)
        commit_field_val = str(
            proof_obj.get("agg_commit_field")
            or proof_obj.get("rsu_commit_field")
            or proof_summary.get("rsu_commit_field")
            or proof_summary.get("global_commit_field")
            or ""
        )

        expected_public_inputs, unresolved = _build_expected_public_inputs_v1(
            public_input_names,
            {
                "root_poseidon_field": int(_as_int(
                    round_summary.get("rsu_root_poseidon_field", proof_summary.get("root_poseidon_field", 0)),
                    0,
                )),
                "anchor_root_poseidon_field": int(_as_int(
                    anchor_ctx.get("anchor_root_poseidon_field", proof_summary.get("anchor_root_poseidon_field", 0)),
                    0,
                )),
                "agg_commit_field": str(commit_field_val),
                "rsu_commit_field": str(commit_field_val),
                "commit_field": str(commit_field_val),
                "pins_hash_field": int(_as_int(
                    proof_summary.get("pins_hash_field", round_summary.get("pins_hash_field", 0)),
                    0,
                )),
                "policy_id_field": int(_as_int(
                    proof_summary.get("policy_id_field", round_summary.get("policy_id_field", 0)),
                    0,
                )),
                "public_input_order_id_field": int(_as_int(
                    proof_summary.get(
                        "public_input_order_id_field",
                        round_summary.get("public_input_order_id_field", 0),
                    ),
                    0,
                )),
            },
        ...
        exported_public_inputs = _extract_public_inputs_from_json_v1(public_sidecar_obj)
        mismatches = _make_mismatch_list(public_input_names, expected_public_inputs, exported_public_inputs)

        rsu_report["proof_summary_path"] = str(proof_summary_path)
        rsu_report["proof_path"] = str(proof_path)
        rsu_report["public_sidecar_path"] = str(public_sidecar_path)
        rsu_report["public_input_names"] = list(public_input_names)
        rsu_report["expected_public_inputs"] = list(expected_public_inputs)
        rsu_report["exported_public_inputs"] = list(exported_public_inputs)
        rsu_report["unresolved_public_input_names"] = list(unresolved)
        rsu_report["mismatches"] = mismatches
        rsu_report["result"] = (
            "pass"
            if public_input_names
            and not unresolved
            and len(expected_public_inputs) == len(exported_public_inputs)
    ...
    try:
        ...
        proof_summary_path = _resolve_output_path_v1(
            output_dir,
            str(global_summary_obj.get("zkp_global_anchor_artifact", "") or ""),
        )
        proof_summary = _read_json_v1(proof_summary_path)

        proof_path = _resolve_output_path_v1(
            output_dir,
            str(proof_summary.get("proof_artifact_path", "") or ""),
        )
        public_sidecar_path = _resolve_output_path_v1(
            output_dir,
            str(
                proof_summary.get("public_inputs_sidecar_path", "")
                or gate_obj.get("global_public_inputs_sidecar_path", "")
                or ""
            ),
        )
        ...
        public_input_names = _spec_public_input_names_v1(spec_global)
        commit_field_val = str(
            proof_obj.get("global_commit_field")
            or proof_obj.get("agg_commit_field")
            or proof_summary.get("global_commit_field")
            or ""
        )
        ...
        expected_public_inputs, unresolved = _build_expected_public_inputs_v1(
            public_input_names,
            {
                "root_poseidon_field": int(_as_int(gate_obj.get("root_poseidon_field", 0), 0)),
                "anchor_root_poseidon_field": int(_as_int(
                    anchor_ctx.get("anchor_root_poseidon_field", proof_summary.get("anchor_root_poseidon_field", 0)),
                    0,
                )),
                "global_commit_field": str(commit_field_val),
                "commit_field": str(commit_field_val),
                "pins_hash_field": int(_as_int(gate_obj.get("pins_hash_field", 0), 0)),
                "policy_id_field": int(_as_int(gate_obj.get("policy_id_field", 0), 0)),
                "public_input_order_id_field": int(_as_int(gate_obj.get("public_input_order_id_field", 0), 0)),
            },
        )
        ...
        global_report["proof_summary_path"] = str(proof_summary_path)
        global_report["proof_path"] = str(proof_path)
        global_report["public_sidecar_path"] = str(public_sidecar_path)
        global_report["public_input_names"] = list(public_input_names)
        global_report["expected_public_inputs"] = list(expected_public_inputs)
        global_report["exported_public_inputs"] = list(exported_public_inputs)
        global_report["unresolved_public_input_names"] = list(unresolved)
        global_report["mismatches"] = mismatches
        global_report["result"] = (
            "pass"
            if public_input_names
            and not unresolved
            and len(expected_public_inputs) == len(exported_public_inputs)
    ...
    out_path = os.path.join(report_dir, "public_input_agreement_report.json")
    _write_json_pretty_v1(out_path, report)
    logging.info("[ABLATION] Saved public-input agreement report to %s", out_path)
    return report
...
if __name__ == "__main__":
    ...
    OUTPUT_DIR = os.getenv("FLBCIDS_ABLATION_V10_2_OUTPUT_DIR", "artifacts/compact_security_evidence/ablation_v10_2")

    ABLATION_CONFIG = {
        # C2 branch 1: invalid signature admission failure
        ...
        # C2 branch 2: mismatched digest binding admission failure
        "enable_digest_binding_failure_check": True,
        "digest_binding_failure_mode": "bad_dp_record_binding",
        "digest_binding_failure_target": {
            "rsu_id": 2,
            "vehicle_id": 2001,
            "round": 1,
        ...
        # Existing non-destructive DP-record tamper probe
        ...
        # Off-chain layered overhead breakdown
    ...
    run_config = {
        ...
        "experiment_seed": 42,
        "publication_role": "compact_security_evidence_ablation_v10_2",
        "script": "FL_DP_SSI_DualMerklePoseidon_RSU+Global_ZKVerifyABLATION_V10_2.py",
