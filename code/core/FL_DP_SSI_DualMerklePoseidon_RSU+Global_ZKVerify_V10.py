#!/usr/bin/env python3
"""
FL-BC-IDS V10 hierarchical DP-XGBoost + SSI + dual-Merkle + Groth16 driver.

Publication/reproducibility notes
---------------------------------
- No machine-specific filesystem path is required.
- Python/data/output locations are controlled through FLBCIDS_* environment
  variables while retaining the documented V10 defaults.
...
# Frozen Merkle/proof parameters
POSEIDON_ARITY = 5
POSEIDON_DEPTH = 32
...
class VehicleClient(fl.client.Client):
    ...
    Each vehicle:
      ...
      - trains/evaluates during FL against the RSU-wide validation split;
    ...
    def fit(self, ins: FitIns) -> FitRes:
        ...
        try:
            ...
            try:
                ...
                best_score = -1.0
            # For continued boosting, best_iteration is indexed over the full
            # booster, so compare the number of rounds actually added in this fit
            # call instead of comparing a global iteration index with the local cap.
            stopped_early = bool(
...
if __name__ == "__main__":
    ...
    # ---------------------------
    DATASET_NAME = os.getenv("FLBCIDS_DATASET_NAME", "CSECICIDS2018").strip()
    DATASET_CONFIGS = {
        "CSECICIDS2018": {
    ...
    # ---------------------------
    # V10 expanded-topology defaults; override through environment variables
    # for documented compact/long-round reproductions without editing source.
    NUM_RSUS = int(os.getenv("FLBCIDS_NUM_RSUS", "10"))
    VEHICLES_PER_RSU = int(os.getenv("FLBCIDS_VEHICLES_PER_RSU", "20"))
    NUM_ROUNDS = int(os.getenv("FLBCIDS_NUM_ROUNDS", "2"))
    NUM_LOCAL_ROUNDS = int(os.getenv("FLBCIDS_NUM_LOCAL_ROUNDS", "10"))
    if min(NUM_RSUS, VEHICLES_PER_RSU, NUM_ROUNDS, NUM_LOCAL_ROUNDS) <= 0:
        raise ValueError(
            "FLBCIDS_NUM_RSUS, FLBCIDS_VEHICLES_PER_RSU, "
            "FLBCIDS_NUM_ROUNDS, and FLBCIDS_NUM_LOCAL_ROUNDS must be positive."
        )
    ...
    ANCHOR_SEED = 42
    ANCHOR_VERSION = "v1"
    ANCHOR_OVERWRITE = os.getenv("FLBCIDS_ANCHOR_OVERWRITE", "1").strip() == "1"
    # DP run output directory (comparison script expects this as dp_dir)
    OUTPUT_DIR = os.getenv("FLBCIDS_DP_OUTPUT_DIR", "artifacts/rsu_outputs_dp")
    # -------------------------------------------------------------------
    ...
    run_config = {
        ...
        "experiment_seed": 42,
        "publication_role": "v10_hierarchical_dp_ssi_zk_driver",
