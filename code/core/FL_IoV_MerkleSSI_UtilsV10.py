...
def _node_modules_dir() -> Optional[Path]:
    """Resolve the Node dependency directory used by circomlibjs."""
    ...
    if root:
        ...
        candidates.extend(
            [
                rp / "node_modules",
                rp / "environment" / "node_modules",
            ]
        )

    module_dir = Path(__file__).resolve().parent
    for cur in (module_dir, *module_dir.parents):
        candidates.append(cur / "node_modules")
        candidates.append(cur / "environment" / "node_modules")
...
def _node_cwd() -> Path:
    """Return a cwd from which Node can resolve ``circomlibjs``."""
    nm = _node_modules_dir()
    if nm is not None:
        return nm.parent
...
def _run_node_json(js_path: Path, payload: Dict[str, Any]) -> Dict[str, Any]:
    ...
    if nm is not None:
        ...
        env["NODE_PATH"] = (
            ...
            else str(nm) + os.pathsep + old_node_path
        )
        nm_bin = nm / ".bin"
        if nm_bin.is_dir():
